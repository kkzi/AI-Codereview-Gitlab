...
class BaseReviewer(abc.ABC):
    ...
    def __init__(self, prompt_key: str, language_file: Optional[str] = None):
        ...
        if language_file:
            self.prompts = self._load_prompts_by_language(
                prompt_key, os.getenv("REVIEW_STYLE", "professional"), language_file
            )
        else:
            self.prompts = self._load_prompts_by_language(
                prompt_key, os.getenv("REVIEW_STYLE", "professional"), None
            )
    ...
    def _load_prompts(self, prompt_key: str, style="professional") -> Dict[str, Any]:
        ...
        """
        logger.warning(
            "使用了已废弃的_load_prompts方法，建议改用_load_prompts_by_language"
        )
    ...
    def _load_prompts_by_language(
        self, prompt_key: str, style="professional", language_file: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if language_file:
            language_prompt_path = os.path.join(
                os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
                "conf",
                f"{language_file}.yml",
            )
            if os.path.exists(language_prompt_path):
                try:
                    with open(language_prompt_path, "r", encoding="utf-8") as file:
                        ...
                        logger.info(
                            f"使用 {language_file}.yml 中的语言特定提示词进行审查"
                        )
                except (FileNotFoundError, KeyError, yaml.YAMLError) as e:
                    logger.warning(f"加载语言特定提示词失败: {e}，将使用基础模板")
            else:
                logger.warning(
                    f"语言特定提示词文件不存在: {language_prompt_path}，将使用基础模板"
                )

        # 使用Jinja2渲染模板
        def render_template(template_str: str) -> str:
            model_name = ReviewService._get_model_name_from_env() or "LLM"
            return Template(template_str).render(style=style, model_name=model_name)
        ...
        try:
            ...
            system_prompt_parts.extend([scoring_rules, output_format, style_template])
    ...
    def call_llm(self, messages: List[Dict[str, Any]]) -> str:
        ...
        logger.info(
            f"✅ 收到 AI 返回结果: {review_result[:200]}..."
        )  # 只记录前200字符避免日志过长
        return review_result
...
class CodeReviewer(BaseReviewer):
    ...
    @staticmethod
    def detect_primary_language_name(changes: Optional[List[Dict[str, Any]]]) -> str:
        language_file = CodeReviewer._detect_language_from_changes(changes)
    ...
    def _detect_language_from_changes(
        ...
        for change in changes:
            ...
            if ext in EXTENSION_TO_PROMPT:
                ...
                language_counts[language_file] = (
                    language_counts.get(language_file, 0) + 1
                )
        ...
        if primary_count / len(changes) >= 0.5:
            logger.info(
                f"检测到主要编程语言: {primary_language} ({primary_count}/{len(changes)} 文件)"
            )
            return primary_language
        else:
            # 混合语言，使用默认提示词
            logger.info(
                f"检测到混合编程语言，主要语言 {primary_language} 占比 {(primary_count / len(changes) * 100):.1f}%，将使用默认提示词"
            )
            return None
    ...
    def _standardize_output_format(self, review_text: str) -> str:
        ...
        # 确保有【评分详情】部分
        if "【评分详情】" not in review_text and (
            "功能实现" in review_text or "/40 分" in review_text
        ):
        ...
        # 去重重复的标题标签（如【总分】【总分】、【结论】【结论】）
        review_text = re.sub(r"(【总分】)+", "【总分】", review_text)
        review_text = re.sub(r"(【结论】)+", "【结论】", review_text)
