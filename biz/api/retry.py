...
def _resolve_gitlab_project_id(base_url: str, token: str, project_path: str) -> int:
    ...
    data = _gitlab_get_json(base_url, token, f"api/v4/projects/{quote(project_path, safe='')}")
    try:
...
def _resolve_gitlab_commit_parent(base_url: str, token: str, project_id: Any, sha: str) -> str:
    if not base_url or not project_id or not sha:
        return ""
    data = _gitlab_get_json(base_url, token, f"api/v4/projects/{project_id}/repository/commits/{quote(sha)}")
...
def trigger_retry(record_id: int, review_type: str) -> Tuple[Dict[str, Any], int]:
    ...
    if review_type == "push":
        ...
        if gitlab_token and base_url and project_path:
            try:
                project_id = _resolve_gitlab_project_id(base_url, gitlab_token, project_path)
                before = _resolve_gitlab_commit_parent(base_url, gitlab_token, project_id, commit_id)
            except Exception:
        ...
        commit_message = (raw_commit_message.split(";", 1)[0] if raw_commit_message else "retry").strip()
        author_name = (record.get("author_display_name") or "").strip() or (record.get("author_username") or "").strip()
